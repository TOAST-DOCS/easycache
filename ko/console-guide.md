...
### 읽기 전용 도메인 설정
* Replica 노드가 추가되어 있는 대상 복제 그룹을 선택한 후 기타 액션 버튼(⋯)을 클릭하고 읽기전용 도메인 설정을 클릭해 읽기 전용 도메인을 설정할 수 있습니다.
...
* Replica 노드에 장애가 발생하거나 Replica 노드를 삭제하는 경우
    ...
    * Replica 노드를 복구하거나 수동으로 삭제한 후 새 Replica 노드를 추가하는 경우, 읽기 전용 도메인은 새 Replica 노드의 IP로 바인딩이 변경됩니다.
        * 바인딩 변경에 실패하면 복제 그룹 > 접속 정보에서 수동으로 재시도할 수 있습니다.
* 읽기 전용 도메인을 설정한 상태에서 Replica 노드 또는 복제 그룹을 삭제하거나 서비스를 비활성화할 수 있습니다.
    * 읽기 전용 도메인을 설정한 상태에서 Replica 노드를 삭제할 경우 읽기 전용 도메인은 Master 노드의 IP로 바인딩이 변경됩니다.
...
생성된 복제 그룹을 선택하고 **노드 정보** 탭을 누르면 복제 그룹 노드의 상세 정보와 로그를 확인하고 Replica 노드를 Master 노드로 승격할 수 있습니다.

![rep_node_info_001.PNG](https://static.toastoven.net/prod_easycache/20.09.01/rep_node_info_002.PNG)
...
##### 제약 사항
* 프로필 생성의 상세 정보 설정과 프로필 수정, 복사 할 경우 설정 정보를 변경 할 수 있습니다.
* 항목값의 범위를 벗어난 값을 입력하거나 적절하지 않은 항목 값을 입력할 경우 장애가 일어날 가능성이 있습니다. 
* 설정 정보를 변경 할 경우 항목값의 범위 또는 적절한 입력값에 대해서는 Redis Document를 참고해 주시길 바랍니다.
...
* 노드 타입 별 재시작에 따른 서비스 영향은 아래와 같습니다.
  * MASTER : 상세 설정→노드 정보→마스터 승격을 이용하여 Master 노드를 Replica 노드로 변경한 뒤 재시작 기능을 이용합니다. 승격없이 Master 노드를 재시작하는 경우 장애 조치(failover) 이벤트가 발생할 수 있습니다
  * REPLICA : 서비스에 영향은 없습니다.
  * HA : 재시작되는 동안에 장애 조치(failover) 보장이 되지 않거나 장애 조치(failover) 이벤트가 발생할 수 있습니다.
 
* 노드 이동으로 인해 서비스에 영향이 있다고 판단될 경우 TOAST 고객 센터로 연락해 주시면 적합한 조치를 안내해 드리겠습니다.

### 4. 점검 대상 복제 그룹의 오른쪽에 표시된 !재시작 아이콘을 클릭하여 점검 노드를 재시작합니다.

![migration_003.png](https://static.toastoven.net/prod_easycache/20.12.01/migration_003.png)

* 점검 노드가 다수일 경우 재시작 한 번 실행에 하나의 노드씩만 재시작 되며, 실행 순서는 HA 노드→Replica 노드→Master 노드입니다.
