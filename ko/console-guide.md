...
### 읽기 전용 도메인 설정
* Replica 노드가 추가되어 있는 대상 복제 그룹을 선택한 후 기타 액션 버튼(⋯)을 클릭하고 **읽기 전용 도메인 설정** 을 클릭해 읽기 전용 도메인을 설정할 수 있습니다.
...
* 컨테이너와 RDB 파일 경로는 영문 대소문자, 숫자, 특수기호 -,_,/,. 만 입력 할 수 있습니다.
...
### 데이터 내보내기
* 복제 그룹을 선택한 후 기타 액션 버튼(⋯)을 클릭하고 **데이터 내보내기**를 클릭해 데이터를 내보낼 수 있습니다.
...
* 컨테이너 경로와 Prefix는 영문 대소문자, 숫자, 특수기호 -,_,/,. 만 입력 할 수 있습니다.
* EasyCache 복제 그룹의 데이터가 1기가 이상일 경우 오브젝트 스토리지로 내보낼 때 세그먼트 오브젝트가 자동으로 만들어집니다.
...
* Replica 노드가 추가되어 있는 대상 복제 그룹을 선택한 후 기타 액션 버튼(⋯)을 클릭하고 **마스터 변경** 을 클릭해 복제그룹의 Master 노드를 변경할 수 있습니다.
...
|             |  재시작 | 시작, 실패, 종료 |
|             |  그룹 인스턴스 변경 | 시작, 실패, 종료 |
|             |  데이터 가져오기 | 시작, 유저 Object Storage Service 설정 실패, 데이터 파일 다운로드 실패, 손상된 파일 또는 지원하지 않는 파일 형식, 노드 재시작 실패, 레플리카 동기화 실패, 종료 |
