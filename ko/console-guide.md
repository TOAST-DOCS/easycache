...
* 버전 업그레이드를 실행하려면 Redis 버전 5인 대상 복제 그룹을 선택한 후 기타 액션 버튼(⋯)을 클릭하고 **버전 업그레이드**을 클릭합니다.
* **버전 업그레이드** 대화 상자에서 버전 업그레이드시 적용될 설정 프로필을 선택할 수 있습니다.
* 버전 업그레이드를 실행하기 전에 데이터 백업을 수행하면 데이터 유실등의 만일의 상황에 대비할 수 있습니다.
...
##### 제약 사항
* 업그레이드하려는 Redis 버전 5인 기존 복제 그룹에 기본 설정 프로필이외의 사용자 설정 프로필이 적용되어 있는 경우, 업그레이드시 동일한 설정 프로필을 적용하려면 **프로필 설정**에서 Redis 버전 6인 사용자 설정 프로필을 미리 생성한 후 버전 업그레이드시 해당 설정 프로필을 선택할 팔요가 있습니다.
* 읽기 전용 도메인이 설정되어 있는 경우는 읽기 전용 도메인 설정을 해제한 후 버전 업그레이드를 할 수 있습니다.
* **standalone** 타입의 복제 그룹의 경우
  * 버전 업그레이드 도중의 복제 그룹에 대한 쓰기는 업그레이드 이후의 데이터 복원 대상에서 제외됩니다.
  * 버전 업그레이드 도중의 Redis 서버 재시작시 일시적으로 복제 그룹에 대한 읽기 및 쓰기를 할 수 없습니다.
* **replication** 타입의 복제 그룹의 경우
  * 버전 업그레이드후 Master 노드가 변경됩니다.
  * 버전 업그레이드 도중 업그레이드에 필요한 마스터 변경이 수행될 때 일시적으로 복제 그룹에 대한 읽기 및 쓰기를 할 수 없습니다.
  * 버전 업그레이드 도중에는 장애 조치(failover) 보장이 되지 않거나 장애 조치(failover) 이벤트가 발생할 수 있습니다.
