...
* 이용 불가 상태의 복제 그룹은 선택할 수 없으며, 복제 그룹의 maxmemory가 백업의 캐쉬 사이즈 보다 커야 합니다.
* 복제 그룹의 버전이 백업이 만들어진 복제 그룹의 버전보다 동일하거나 높은 복제 그룹에만 복원 할 수 있습니다.
...
4. **메트릭 조건**과 **이벤트 조건**에 필요한 설정을 결정하기 힘든 경우에는 **알람 템플릿** 에서 제공하는 기본 템플릿을 이용하는 것을 고려해주세요.
...
|             |  기존 복제 그룹 복원 | 시작, 데이터 파일 다운로드 실패, 손상된 파일 또는 지원하지 않는 파일 형식, 노드 재시작 실패, 레플리카 동기화 실패, 종료 |
|             |  HA 설정 갱신 | 시작, 실패, 종료 |
|             | 마스터 변경 | 시작, 실패, 종료 |
| **공인 도메인** | 설정 | 시작, 실패, 종료 |
|             | 해제 | 시작, 실패, 종료 |
