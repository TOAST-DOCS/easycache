...
|             | 修正  | 開始、失敗、終了|
|             | 再起動 | 開始、失敗、終了 |
|             | データインポート | 開始、ユーザーOBS設定失敗、データファイルダウンロード失敗、損傷したファイルまたはサポートしないファイル形式、ノード再起動失敗、レプリカ同期失敗、終了 |
|             |  HA設定更新 | 開始、失敗、終了 |
...
* 各ノードタイプで再起動に伴うサービスへの影響は下記のとおりです。
  * MASTER：詳細設定→ノード情報→マスター昇格を選択してMasterノードをReplicaノードに変更した後、再起動機能を利用できます。Standaloneの場合は昇格しないで利用できます。
